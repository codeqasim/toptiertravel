...
// ======================================================== LISTING PAGE
$router->get('(flights)/(.*)', function($nav_menu,$url)  {
    ...
    if ($uri[2] == "oneway"){
        $_SESSION['flights_origin']=$uri[0];
        $_SESSION['flights_destination']=$uri[1];
        $_SESSION['flights_type']=$uri[2];
        $_SESSION['flights_class']=$uri[3];
        $_SESSION['flights_departure_date']=$uri[4];
        $_SESSION['flights_adults']=$uri[5];
        $_SESSION['flights_childs']=$uri[6];
        $_SESSION['flights_infants']=$uri[7];
    }
    ...
    if ($uri[2] == "return"){
        ...
        $_SESSION['flights_class']=$uri[3];
        $_SESSION['flights_departure_date']=$uri[4];
        $_SESSION['flights_returning_date']=$uri[5];
        $_SESSION['flights_adults']=$uri[6];
        $_SESSION['flights_childs']=$uri[7];
        $_SESSION['flights_infants']=$uri[8];
    }

    // IF FLIGHT HAS NO TYPE MAKE IT ONEWAY
    if (!isset($_SESSION['flights_type'])){
        $_SESSION['flights_type'] = "oneway";
    }
...
$router->post('(flight)/booking', function($nav_menu) {
...
$router->post('(flight)/book', function() {
    ...
    $total_price_markup = floatval($agent_fee) + floatval($routes->segments[0][0]->price);
