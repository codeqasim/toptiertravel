<link rel="stylesheet" type="text/css" href="//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.css"/>
<script src="
https://cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.js
"></script>

<section class="feature cars">
<section class="container" style="border-radius:10px;padding:50px 0px">
<div class="container">
<h2 class="text-center text-white mt-5 text-capitalize">

<!-- <svg class="d-block text-center mx-auto mb-2" fill="" width="50" height="50" viewBox="0 -3.97 122.88 122.88" version="1.1" id="Layer_1"
xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
style="enable-background:new 0 0 122.88 114.93" xml:space="preserve">
<style type="text/css">
.st0 {
    fill-rule: evenodd;
    clip-rule: evenodd;
}
</style>
<g>
    <path class="st0"
        d="M117.95,53.71v41.82v9.94v9.46h-14.31v-9.46h-84.4v9.46H4.93v-9.46v-9.94V53.71H0v-9.41h12.04l6.71-20.88 c1.98-6.15,5.28-11.74,11.74-11.74h14.78V3.4c0-3.71-0.77-3.39,2.72-3.39h27.33c2.92,0,2.29-0.28,2.29,2.83v8.83h17.28 c6.46,0,10.18,5.48,11.74,11.74l5.19,20.88h11.06v9.41H117.95L117.95,53.71z M40.01,86.58h42.27v9.97H40.01V86.58L40.01,86.58z M9.72,63.36c10.77,0.34,17.36,4.86,19.05,14.26H9.72V63.36L9.72,63.36z M111.8,63.36c-10.77,0.34-17.36,4.86-19.05,14.26h19.05 V63.36L111.8,63.36z M17.18,44.29h88.52l-3.79-17.51c-1.04-4.8-4.03-8.95-8.95-8.95H31.74c-4.92,0-7.44,4.26-8.95,8.95L17.18,44.29 L17.18,44.29z" />
</g>
</svg> -->

<strong><?=T::cars_rentalbestcarstoday?></strong></h5>
<div style="padding:50px 20" id="fadein">
<div class="p-5 rounded bg-white shadow-lg mt-4">
<?php include "Search.php"; ?>
</div>
</div>
</div>
</section>
</section>

<svg style="position: relative;bottom: 0;left: 0;width: 100%;height: 50px;fill: #fff;z-index: 100;margin-top: -80px;"
class="hero-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 10" preserveAspectRatio="none">
<path d="M0 10 0 0 A 90 59, 0, 0, 0, 100 0 L 100 10 Z"></path>
</svg>

<?php include "Featured.php"; ?>
<style>
.select2-container--default .select2-selection--single .select2-selection__rendered {
padding: 0px 7px !important;
}
</style>