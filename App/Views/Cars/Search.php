<form id="cars-search" class="content m-0 search_box">
        ...
        <div class="input-items">
            ...
            <div class="form-floating">
                ...
                <select id="" name="" class="cars_flights_origin form-control w-100" required>
                    <?php if(isset($_SESSION['from_airport'])){ ?>
                    <option value="<?=strtoupper($_SESSION['from_airport'])?>" selected><?=strtoupper($_SESSION['from_airport'])?></option>
                    <?php } else {?>
                    <option value="Select City"><?=T::select?></option>
                    <?php } ?>
                </select>
                <label style="margin: 0 24px" for=""><?=T::from_airport?></label>
            </div>
        </div>
   ...
   <div class="col-md-6">
        <div class="input-items cars_location">
            ...
            <div class="form-floating">
                <select id="" name="city" class="origin cars_city form-control w-100" required>
                    <?php if(isset($_SESSION['to_location'])){ ?>
                    <option value="<?=strtoupper($_SESSION['to_location'])?>" selected><?=strtoupper($_SESSION['to_location'])?></option>
                    <?php } else {?>
                    <option value="Select City"><?=T::select?></option>
                    <?php } ?>
                </select>
                <label style="margin: 0 24px" for=""><?=T::to?> <?=T::location?></label>
            </div>
        </div>
        <div class="d-block d-sm-none"></div>
    </div>
      ...
      <div class="col-lg-3">
        <div class="d-inline-flex w-100 input-group" style="flex-wrap: unset;">
            <div class="form-floating" style="width:220px">
                <input name="date" class="dp form-control" id="cars_from_date" type="text" placeholder="" value="<?php if(isset($_SESSION['to_date'])){ echo $_SESSION['to_date']; } else { $d=strtotime("+3 Days"); echo date("d-m-Y", $d); } ?>" readonly="readonly"/>
                <label for="checkin">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="form-floating w-100">
                ...
                <?php
                    for ($i = 0; $i < 24; $i++) {
                        $hour24 = str_pad($i, 2, '0', STR_PAD_LEFT);
                        $period = $i < 12 ? 'AM' : 'PM';
                        $picktime = isset($_SESSION['pick_time']) ? $_SESSION['pick_time'] : '';
                        $get_hour = ($hour24 . ':00' === $picktime);
                        $get_minute = ($hour24 . ':30' === $picktime);
                        // Display options with 24-hour format and AM/PM
                        echo "<option value=\"{$hour24}:00\"" . ($get_hour ? ' selected' : '') . ">{$hour24}:00 {$period}</option>";
                        echo "<option value=\"{$hour24}:30\"" . ($get_minute ? ' selected' : '') . " >{$hour24}:30 {$period}</option>";
                    }
                    ?>
                </select>
      ...
      <div class="col-lg-3">
        <div class="d-inline-flex w-100 input-group" style="flex-wrap: unset;">
            <div class="form-floating" style="width:220px">
                <input name="date" class="dp form-control" id="cars_to_date" type="text" placeholder="" value="<?php if(isset($_SESSION['drop_date'])){ echo $_SESSION['drop_date']; } else { $d=strtotime("+5 Days"); echo date("d-m-Y", $d); } ?>" readonly="readonly"/>
                <label for="checkin">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect>
                    <line x1="16" y1="2" x2="16" y2="6"></line>
            ...
            <div class="form-floating w-100">
                ...
                <?php
                    ...
                    for ($i = 0; $i < 24; $i++) {
                        ...
                        $droptime = isset($_SESSION['drop_time']) ? $_SESSION['drop_time'] : '';
                        $get_hour = ($hour24 . ':00' === $droptime);
                        $get_minute = ($hour24 . ':30' === $droptime);
                        // Display options with 24-hour format and AM/PM
                        echo "<option value=\"{$hour24}:00\"" . ($get_hour ? ' selected' : '') . ">{$hour24}:00 {$period}</option>";
                        echo "<option value=\"{$hour24}:30\"" . ($get_minute ? ' selected' : '') . " >{$hour24}:30 {$period}</option>";
                    }
      ...
      <div class="col-lg-5">
         <div class="input-box">
            ...
            <div class="dropdown dropdown-contain">
                        ...
                        <?=T::travellers?> <span class="guest_cars"></span></p>
                        </a>
...
<template id="most--popular-cars-origin">
   <div class="most--popular-cars-origin">
      ...
      <?php foreach (base()->cars_suggestions as $s) {
            ?>
...
$("#cars-search").submit(function() {
    ...
    $('.loading_button').hide();
        ...
        var finalURL = '<?=root?>cars/'+trip_from+'/'+trip_to+'/'+cars_from_date+'/'+cars_from_time+'/'+cars_to_date+'/'+cars_to_time+'/'+cars_adults+'/'+cars_child;
...
$ajax.select2({
    ajax: {
        url: "<?=root.api_url?>flights_locations",
        dataType: "json",
        data: function(t) {
            return {
                city: $.trim(t.term)
