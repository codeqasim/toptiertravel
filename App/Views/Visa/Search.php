   ...
   <div class="row mb-0 pt-1 g-2">
    ...
    <div class="col-md-4">
         <div class="input-items">
            ...
            <label style="margin: 5px 24px" for=""><?=T::from?> <?=T::country?></label>
            </div>
      ...
      <div class="col-md-4">
         <div class="input-items">
            ...
            <label style="margin: 5px 24px" for=""><?=T::to?> <?=T::country?></label>
            </div>
...
<script>
      ...
      // Initialize the select2 on the select element
      $('.visa_to').select2({ placeholder: { id: '1', text: 'Select Category' } });
      $('.visa_from').select2({ placeholder: { id: '1', text: 'Select Category' } });

      // Function to fetch countries and populate the select2
      function fetchCountriesFrom() {
        $.ajax({
          url: '<?=api_url?>/visa_from_countries', // Replace with your API endpoint
          method: 'POST',
          contentType: 'application/json',
          success: function(response) {
            // Iterate over the response to populate countries
            response.forEach(function(country) {
              // Create an option element and append it to the select
              $('.visa_from').append(
                $('<option>', {
                  value: country.iso, // Use ISO code as the value
                  text: country.nicename // Use the nicename for display
                })
              );
            });

            // Refresh the select2 to reflect the added options
            $('.visa_from').trigger('change');
            ...
            var user_visa_country = c['country_code'];

               if( typeof user_visa_country !== 'undefined' ) {
                  // console.log(user_visa_country);
                  // MAKE DEFAULT COUNTRY
                  $('.visa_from').val(user_visa_country).trigger('change');
               }
      ...
      // Call the function to fetch and populate countries
      fetchCountriesFrom();


      // Function to fetch countries and populate the select2
      function fetchCountriesTo() {
        $.ajax({
          url: '<?=api_url?>/visa_to_countries', // Replace with your API endpoint
          method: 'POST',
          contentType: 'application/json',
          success: function(response) {
            // Iterate over the response to populate countries
