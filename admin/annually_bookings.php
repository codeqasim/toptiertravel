...
// INITIALIZE MONTHS
$yearMonths = [];
$currentMonth = $firstDayOfYear;
while (strtotime($currentMonth) <= strtotime($lastDayOfYear)) {
...
// FETCH DATA FOR EACH MONTH IN THE CURRENT YEAR
foreach ($yearMonths as $month => &$counts) {
    $monthStart = date('Y-m-01', strtotime($month));
    $monthEnd = date('Y-m-t', strtotime($month));

    foreach (['Cars', 'Flights', 'Hotels', 'Tours'] as $type) {
        $counts[$type]['total'] = $db->count(strtolower($type) . '_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd]]);
        $counts[$type]['unpaid'] = $db->count(strtolower($type) . '_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd], 'payment_status' => 'unpaid']);
        $counts[$type]['paid'] = $db->count(strtolower($type) . '_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd], 'payment_status' => 'paid']);
        $counts[$type]['refunded'] = $db->count(strtolower($type) . '_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd], 'payment_status' => 'refunded']);
        $counts[$type]['disputed'] = $db->count(strtolower($type) . '_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd], 'payment_status' => 'disputed']);
        $counts[$type]['pending'] = $db->count(strtolower($type) . '_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd], 'booking_status' => 'pending']);
        $counts[$type]['confirmed'] = $db->count(strtolower($type) . '_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd], 'booking_status' => 'confirmed']);
        $counts[$type]['cancelled'] = $db->count(strtolower($type) . '_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd], 'booking_status' => 'cancelled']);
    }
    
    // ADD DATA FETCHING FOR VISA BOOKINGS
    $counts['Visa']['total'] = $db->count('visa_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd]]);
    $counts['Visa']['unpaid'] = $db->count('visa_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd], 'booking_payment_status' => 'unpaid']);
    $counts['Visa']['paid'] = $db->count('visa_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd], 'booking_payment_status' => 'paid']);
    $counts['Visa']['refunded'] = $db->count('visa_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd], 'booking_payment_status' => 'refunded']);
    $counts['Visa']['disputed'] = $db->count('visa_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd], 'booking_payment_status' => 'disputed']);
    $counts['Visa']['pending'] = $db->count('visa_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd], 'booking_status' => 'pending']);
    $counts['Visa']['confirmed'] = $db->count('visa_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd], 'booking_status' => 'confirmed']);
    $counts['Visa']['cancelled'] = $db->count('visa_bookings', ['booking_date[<>]' => [$monthStart, $monthEnd], 'booking_status' => 'cancelled']);
}
...
$chartData = [];
foreach ($yearMonths as $month => $counts) {
    $monthLabel = date('M', strtotime($month)); // Use 'M' for short month names (e.g., 'Jan', 'Feb')
    $chartData[] = [
        'month' => $monthLabel,
...
am5.ready(function() {
    ...
    function createSeries(name, totalField, unpaidField, paidField, refundedField, disputedField, pendingField, confirmedField, cancelledField, color) {
        var series = chart.series.push(am5xy.ColumnSeries.new(root, {
            name: name,
            xAxis: xAxis,
            yAxis: yAxis,
            valueYField: totalField,
    ...
    createSeries("Visa", "visa_total", "visa_unpaid", "visa_paid", "visa_refunded", "visa_disputed", "visa_pending", "visa_confirmed", "visa_cancelled", am5.color(0x9467bd)); // Added Visa series
