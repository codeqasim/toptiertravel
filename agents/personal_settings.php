...
$user_id = (DECODE($_SESSION['phptravels_backend_user'])->backend_user_id);
...
if (isset($_POST['status'])) { $STATUS = 1; } else { $STATUS = 0; };
    ...
    if (isset($exist_mail[0]['email'])) {
      ...
      if($exist_mail[0]['user_id']!==$user_id){ ?>

        <div class="container py-3">
          <p>This email already exist with another user. please use unique email address</p>
...
<div class="col-lg-6">
  <div class="card card-raised mb-3">
    <div class="card-body p-4">
      ...
      <!-- <div class="form-floating mb-3 user_roles">
      ...
      <?php } else {
        // echo "<div class='border rounded-3 p-4 mb-3'><strong>";
        // echo $profile->user_type;
        // echo "</strong></div>";
        // echo '<input name="user_type" type="hidden" value="'.$profile->user_type.'" />';
      } 
      ?>
...
<div class="col-lg-6">
   ...
   <div class="card mt-3 user-wallet">
      <div class="card-body p-4">
...
<div class="card user_payment_details">
    <div class="card-body p-4">
        <div class="card-title"><strong><?=T::payment?> <?=T::details?></strong></div>
        <div class="border p-2 px-4 rounded-2">
            <label for="payment_method" class="form-label">Select Payment Method</label>
            <select id="payment_method" class="form-select" onchange="showFields()">
                <option value="paypal">PayPal</option>
                <option value="venmo">Venmo</option>
                <option value="zelle">Zelle</option>
                <option value="wire">Wire</option>
            </select>
            <div id="payment_fields" class="mt-3"></div>
        </div>
    </div>
</div>
...
<script>
    function showFields() {
        ...
        const fieldsContainer = document.getElementById('payment_fields');
        let fields = '';
        
        if (method === 'paypal') {
            fields = `<label>PayPal Email</label><input type="email" class="form-control" required>`;
        } else if (method === 'venmo') {
            fields = `<label>Venmo</label><input type="text" class="form-control" placeholder="@username" required>`;
        } else if (method === 'zelle') {
            fields = `<label>Zelle Contact</label><input type="text" class="form-control" placeholder="Phone or Email" required>`;
        } else if (method === 'wire') {
            fields = `<a href="https://stripe.com/connect" target="_blank" class="btn btn-primary">Set up Wire Transfer with Stripe</a>`;
        }
