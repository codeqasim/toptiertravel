...
function MakeDefaultCurrency($value, $fieldname, $primary_key, $row, $xcrud){
...
function create_lang($postdata, $primary, $xcrud)
{
    ...
    foreach ($languages as $key=>$value){
        ...
        //Add Cms Pages Languages
        $blogs_query = 'SELECT * FROM cms';
        $db->query($blogs_query);
        $blogs_query = $db->result();
        foreach ($blogs_query as $bdata){
            $check_bdata = "SELECT * FROM `cms_translations` WHERE `page_id` = ".$bdata['id']." and `language_id` = ".$value['id'];
            $db->query($check_bdata);
            $tran_bcheck = $db->result();
            if(empty($tran_bcheck)) {
                $tran_blog_query = 'INSERT INTO `cms_translations`( `page_id`, `language_id`) VALUES (' . $bdata['id'] . ',' . $value['id'] . ')';
                $db->query($tran_blog_query);
            }
        }
    }
