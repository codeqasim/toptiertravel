...
if (isset($hook)) {
    ...
    if ($hook == "hotels/update_booking") {
        ...
        // SEND EMAIL
        $title = "Payment Completed";
        $template = "hotel_booking_confirmatoin";
    ...
    // ==================================================================== VISA SUBMISSION
    if ($hook == "visa_submissions") {

        // SEND EMAIL
        $title = "Visa Submission";
        $template = "visa_submission";
        $content = '';
        $receiver_email = ($email);
        $receiver_name = ($name);
        MAILER($template, $title, $content, $receiver_email, $receiver_name);
        ...
        // PUSH NOTIFICATION
        $push_data['message1'] = 'Visa Submission';
        $push_data['message2'] = 'New visa submission request' . $email;
        $pusher->trigger($channel, 'event', $push_data);
