        ...
        if (!empty($c_response) && isset($c_response['response']) && is_array($c_response['response'])) {
            ...
            foreach ($c_response['response'] as $values) {
                ...
                $curl_response[] = [
                    ...
                    "actual_price" => number_format($values['price'], 2),
                    "actual_price_per_night" => number_format($values['price_per_night'] / $days, 2),
                    "markup_price" => number_format($values['price'] + $markup_cprice, 2),
                    "markup_price_per_day" => number_format($values['price_per_night'] + $markup_cprice / $days, 2),
                    "currency" => $currency_id,
                    "booking_currency" => $currency_id,
