...
// Calculate the offset based on the page number
        ...
        $manual_tours = $db->select('tours', '*', ['location' => $params['location'], 'status' => 1,'LIMIT' => [$offset, $items_per_page]]);
            ...
            foreach ($manual_tours as $value) {
                ...
                if ($con_price != null) {
                    ...
                    if(!empty($_POST['price_from']) && !empty($_POST['price_to'])){
                        if ( round($markup_price) < $_POST['price_from'] ||  round($markup_price) > $_POST['price_to']) {
    ...
    $data_tours = [];
            $getvalue = gettourmoduledata($_POST['module_name']); // Get module data for each active module
            ...
            $param = array(
                "endpoint" => api_modules ."/tours/" . strtolower($module_name) . "/api/v1/search",
                "location" => $_POST['location'],
                "date" => $_POST['date'],
                "adults" => $_POST['adults'],
                "childs" => $_POST['childs'],
    ...
    if (!empty($manual_response) && !empty($data_tours)) {
        ...
        $data = ['status'=>true,'response'=>$manual_response,'total'=>count($manual_response)];
    } elseif (!empty($data_tours)) {
        $data = ['status'=>true,'response'=>$data_tours,'total'=>$tours_rep->total];
    } else {
        $data = array("status" => false, "message" => "No Tours data", "response" => (object)[],'total'=>'');
    }
...
$router->post('tour/details', function () {
    ...
    if ($tour) {
        $tour[0]['tour_meta_desc'] = preg_replace('/[®+]/', '', $tour[0]['tour_meta_desc']);
        $tour[0]['tour_meta_keywords'] = preg_replace('/[®+]/', '', $tour[0]['tour_meta_keywords']);
    }
    $module_id = $db->select('modules', ['id'], ['name' => $_POST['supplier'], 'type' => 'tours', 'status' => 1]); //GET THE MODULE ID
    ...
    if (!empty($tour) && !empty($module_id[0]['id'])) {
        ...
        if (!empty($tour_img)) {
            foreach ($tour_img as $value) {
                $tour_imgs[] = root.'../uploads/'.$value['img'];
            }
        }
        array_unshift($tour_imgs,root.'../uploads/'.$tour[0]['img']);
        ...
        $get_tour = gettourmoduledata($_POST['supplier']);
        $supplier_name = $get_tour[0]['name'];
        $supplier_id = $get_tour[0]['id'];
        // Determine whether the module is in development or production mode
        if ($get_tour[0]['dev_mode'] == 1) {
            $env = 'production';
        } else {
            $env = 'dev';
        }
        ...
        //VALIDATION
        $param = array(
            "endpoint" => api_modules ."/tours/".strtolower($supplier_name)."/api/v1/detail",
            "tour_id" => $_POST['tour_id'],
            "date" => $_POST['date'],
            "adults" => $_POST['adults'],
            "childs" => $_POST['childs'],
        ...
        if(empty($get_tour[0]['c1'])) {
            include "creds.php";
        }

        $curl = sendtourRequest('POST', 'detail', $param); //SENDS A CURL REQUEST TO FETCH THE HOTEL DETAIL RESPONSE ACCORDING TO GIVEN SUPPLIER NAME

        $tour_details = json_decode($curl); //DECODE THE FETCHED RESPONSE

        if(!empty($tour_details->tour_id)){

            //PRICE CALCULATION ON THE BASE OF ADULTS AND CHILDS
            $price = ($_POST['adults'] * $tour_details->adult_price) + ($_POST['childs'] * $tour_details->child_price);

            // PRICES CONVERSIONS
            $con_price = ($price) ? price_con( $price, $_POST['currency']) : 0;
            $con_adult_price = ($tour_details->adult_price) ? price_con($tour_details->adult_price, $_POST['currency']) : 0;
            $con_child_price = ($tour_details->child_price) ? price_con($tour_details->child_price, $_POST['currency']) : 0;

            //MARKUP PRICES
            $loc_code = $db->select('locations', ['id'], ['city' => $tour_details->location]); //GETS THE LOCATION_ID FROM DATABASE FOR MARKUPS
            $markup = toursmarkup($supplier_id, $con_price, $_POST['date'], $loc_code[0]['id']);

            $adult_markup_price = toursmarkup($supplier_id, $con_adult_price, $_POST['date'], $loc_code[0]['id']);
            $child_markup_price = toursmarkup($supplier_id, $con_child_price, $_POST['date'], $loc_code[0]['id']);
            $price_markup = ($markup['b2c'] != 0) ? $markup['b2c'] :$markup['b2b'] ; //CHECKS WHICH MARKUP PRICE IS AVAIABLE

            $response = [
                'tour_id' => $tour_details->tour_id,
                'name' => $tour_details->name,
                'tour_type' => $tour_details->tour_type,
                'date' => $_POST['date'],
                'location' =>  $tour_details->location,
                'img' =>  $tour_details->img,
                'desc' => $tour_details->desc,
                'tours_itinerary' => $tour_details->tours_itinerary,
                'price' => number_format((float)$price_markup, 2, '.', ''),
                'actual_price' => number_format($con_price, 2, '.', ''),
                'b2c_price_adult' => number_format($adult_markup_price['b2c'], 2, '.', ''),
                'b2b_price_adult' => number_format($adult_markup_price['b2b'], 2, '.', ''),
                ...
                'adult_price' => number_format($con_adult_price, 2, '.', ''),
                'child_price' => number_format($con_child_price, 2, '.', ''),
                'max_Adults' => $tour_details->max_Adults,
                'max_Child' => $tour_details->max_Child,
                'max_Infant' => $tour_details->max_Infant,
                'rating' => $tour_details->rating,
                'longitude' => $tour_details->longitude,
                'latitude' => $tour_details->latitude,
                'redirect' => $tour_details->redirect,
                'inclusions' => $tour_details->inclusions,
                'exclusions' => $tour_details->exclusions,
                'currencycode' => $_POST['currency'],
                'duration' => $tour_details->duration,
                'policy' =>$tour_details->policy,
                'max_travellers' => $tour_details->max_travellers,
                'departure_time' => "",
                'departure_point' => "",
